...
locals {
  # Add an owner tag as to filter resources
  tags = "${  merge(var.tags, 
              map("owner", "${var.namespace}"))
          }"
          
  # KubernetesCluster must match the variables values
  # reference for this extra tag https://github.com/kubernetes/kubernetes/issues/29298#issuecomment-274400650  
  #
  ...
  eks_tags = "${  merge(var.tags, 
                  map("owner", "${var.namespace}"), 
                  map("KubernetesCluster", "${var.namespace}-${var.stage}-${var.name}-cluster"), 
